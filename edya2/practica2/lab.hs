...
not2 b = case b of
    True -> False
    False -> True {- Error de indentación -}

-- b)
init2 :: [a] -> [a]
init2 [x] = []
init2 (x : xs) = x : init2 xs
init2 [] = error "empty list" {- Error de modulo con Prelude -}

-- c)
length2 :: [a] -> Int
length2 []        =  0
length2 (_:l)     =  1 + length2 l {- Error de modulo con Prelude -}

-- d)
list123 = 1 : (2 : 3 : []) {- Lo que pasa, es que el operador `:` es del tipo (:) :: a -> [a] -> [a], como segundo parametro troma una lista, entonces al hacer (1:2) estaría tomando como segundo argumento un número -}

-- e)
(++!) :: [a] -> [a] -> [a]
[]     ++! ys = ys
(x:xs) ++! ys = x : xs ++! ys {- No errores -}

-- f)
addToTail :: Int -> [Int] -> [Int]
addToTail x xs = map (+x) (tail xs) {- No errores -}

-- g)
listmin :: Ord a => [a] -> a
listmin xs = (head . sort) xs {- El operador `.` recibe dos funciones y las compone, pero debe ser de la manera (func1 . func2) arg o sino func1 (func2 arg) -}

-- h) (*)
smap :: (a -> a) -> [a] -> [a]
smap f [] = []
smap f [x] = [f x]
smap f (x:xs) = f x : smap f xs {- Llamaba smap (smap f) xs, no tenía logica -}
...
cuadrupla n = [(a, b, c, d) | a <- [0..n], b <- [0..n], c <- [0..n], d <- [0..n], (a^2) + (b^2) == (c^2) + (d^2), not (a==c) && not (a==d)] {- Agregué la última condición para que no me muestre los valores triviales -}
...
scalarProduct [] [] = 0
scalarProduct (x : xs) (y : ys) = (x+y) + scalarProduct xs ys -} {- Está bien, pero vamos a usar listas por compresión así usamos las sugerencias del ej -}
scalarProduct :: [Int] -> [Int] -> Int
