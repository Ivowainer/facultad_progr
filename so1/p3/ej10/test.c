...
int main()
{
    ...
    sleep(1); // Asegura que el escritor complete

    // Creamos lector 2 (se quedará esperando porque no hay otro escritor)
    pthread_create(&r2, NULL, reader, &reader_ids[1]);

    pthread_join(r1, NULL);
    pthread_join(w1, NULL);

    // Esperamos unos segundos y como no hay writers se queda esperando
    sleep(2);
